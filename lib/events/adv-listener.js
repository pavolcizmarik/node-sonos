...
class SonosListener extends EventEmitter {
  ...
  async subscribeTo (device) {
    return new Promise((resolve, reject) => {
      ...
      const sub = new DeviceSubscription(device)
      const add = function (endpoint) {
        return sub.addSubscription(endpoint)
      }
      if (this._deviceSubscriptions.length === 0) {
        const addAll = globalEventEndpoints.concat(deviceEventEndpoints).map(add)
        Promise.all(addAll)
          .then(result => {
            this._deviceSubscriptions.push(sub)
          }).then(resolve).catch(reject)
      } else {
        const addDevice = deviceEventEndpoints.map(add)
  ...
  _handleMessage (req, resp, body) {
    ...
    if (method.toUpperCase() !== 'NOTIFY' || url.toLowerCase() !== '/notify') {
      ...
      const deviceSubscription = this._deviceSubscriptions.find((el, index, arr) => {
      ...
      if (!deviceSubscription) {
        ...
        parseXml(body)
          .then(parsedBody => {
            const endpoint = deviceSubscription.subscriptions[headers.sid].endpoint
            const eventBody = parsedBody['e:propertyset']['e:property']
  ...
  async _handleGlobalNotification (endpoint, body) {
    ...
    const sonosEvent = await ParseAndEmitEvents(endpoint, body, this._deviceSubscriptions[0].device)
...
const defaultListener = new SonosListener()
...
class DeviceSubscription {
  ...
  async renewAllSubscriptions () {
    ...
    const renewAll = Object.keys(this.subscriptions).map(renew)
  ...
  async handleNotification (endpoint, body) {
    ...
    const sonosEvent = await ParseAndEmitEvents(endpoint, body, this.device)
  ...
  async cancelAllSubscriptions () {
    ...
    const cancelAll = Object.keys(this.subscriptions).map(cancel)
    return Promise.all(cancelAll)
  }
  ...
  headerToDateTime (timeout) {
    ...
    seconds = (timeout && !isNaN(timeout)) ? parseInt(timeout, 10) : 3600
    seconds = Math.max(Math.min(seconds - 15, 1200), 15)
