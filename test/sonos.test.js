...
describe('Async DeviceDiscovery', function () {
  it('should reject after timeout without devices found', function () {
    const failTimeout = setTimeout(function () {
      assert(false, 'Event never fired')
    }, 100)

    const deviceDiscovery = new SONOS.AsyncDeviceDiscovery()
    return deviceDiscovery.discover({ timeout: 10 })
      .then((device, model) => {
        clearTimeout(failTimeout)
        assert.fail('Expected error to be handled')
      })
      .catch(err => {
        clearTimeout(failTimeout)
        assert.strictEqual(err.message, 'No devices found')
      })
  })
})
